...
`I18nProvider` creates a reactive i18n context using `createFluentiContext()` internally and provides it to the component tree via Solid's context API. All child components can access it through `useI18n()`.

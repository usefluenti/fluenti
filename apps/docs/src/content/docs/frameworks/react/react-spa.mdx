...
<Trans tag="p">Click <a href="/next">here</a> to continue.</Trans>
...
|------|------|-------------|
| `children` | `ReactNode` | Source text with embedded components |
| `id` | `string` | Override the auto-generated hash ID |
| `context` | `string` | Message context for identity and translator disambiguation |
| `comment` | `string` | Context comment for translators |
| `tag` | `keyof JSX.IntrinsicElements` | Wrapper element tag (e.g. `'span'`, `'p'`). Defaults to Fragment (no wrapper). |
...
function OrderSummary({ date, total }: { date: Date; total: number }) {
  return (
    <div>
      <p>Order placed: <DateTime value={date} format="long" /></p>
      <p>Total: <NumberFormat value={total} format="currency" /></p>
    </div>
  )
}
```
...
| `i18n` | `FluentiInstanceExtended` | Core instance (escape hatch for advanced use) |

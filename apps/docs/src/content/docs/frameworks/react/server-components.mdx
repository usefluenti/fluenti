...
React Server Components render on the server and never ship JavaScript to the client. Fluenti supports RSC via `import { t } from '@fluenti/react'` with `withFluenti()`, or via `createServerI18n` from `@fluenti/react/server`.
...
When using `withFluenti()`, imported `t` works directly in `'use server'` files — the plugin detects the directive and injects the server-side i18n accessor:
...
<Aside type="caution">
  **`router.refresh()` is required.** After changing the locale, the server layout must re-render to update `<html lang>` and the server-side `I18nProvider`. Without `refresh()`, only client components update.
</Aside>
...
| Client Component | Reads from `I18nProvider` context (set by `I18nProvider`) |

...
The `v-t` directive is processed at compile time by the Vite plugin's `nodeTransform`. It never reaches the browser — it's converted to `$t()` calls during the build. See [How It Works](/start/how-it-works/) for the full compilation pipeline.

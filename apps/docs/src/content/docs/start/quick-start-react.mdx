...
`@fluenti/react/components` remains available when you want an explicit components-only subpath, but the default authoring path is the main `@fluenti/react` entry.

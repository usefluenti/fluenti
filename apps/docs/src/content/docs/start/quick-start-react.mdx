...
4. **Write your component — just use `t` and `<Trans>`**
   ...
   export function Home() {
     ...
     return (
       <div>
         {/* t supports both function calls and tagged templates */}
         <h1>{t`Welcome to my app`}</h1>
         <p>{t`Hello, ${name}!`}</p>
         <p>{t('You have {count} items', { count })}</p>
   ...
   No manual keys. No JSON files. `t` from `useI18n()` works as both a function call and a tagged template — the Vite plugin optimizes tagged templates at build time via AST scope analysis.

---
title: Quick Start (SolidJS)
description: Get started with Fluenti in a SolidJS project
---

import { Steps } from '@astrojs/starlight/components'

<Steps>

1. **Install packages**

   ```bash
   pnpm add @fluenti/core @fluenti/solid
   pnpm add -D @fluenti/cli
   ```

2. **Configure Vite**

   ```typescript
   // vite.config.ts
   import { defineConfig } from 'vite'
   import solidPlugin from 'vite-plugin-solid'
   import fluentiSolid from '@fluenti/solid/vite-plugin'

   export default defineConfig({
     plugins: [
       solidPlugin(),
       fluentiSolid(),
     ],
   })
   ```

3. **Create `fluenti.config.ts`**

   ```typescript
   // fluenti.config.ts
   import { defineConfig } from '@fluenti/cli'

   export default defineConfig({
     sourceLocale: 'en',
     locales: ['en', 'zh-CN'],
     catalogDir: './locales',
     format: 'po',
     include: ['./src/**/*.{tsx,ts}'],
     compileOutDir: './src/locales/compiled',
   })
   ```

4. **Write your component — just use `` t`` `` and `<Trans>`**

   ```tsx
   // src/Home.tsx
   import { t, useI18n, Trans, Plural } from '@fluenti/solid'
   import { createSignal } from 'solid-js'

   export function Home() {
     const { setLocale } = useI18n()
     const [name] = createSignal('World')
     const [count] = createSignal(3)

     return (
       <div>
         <h1>{t`Welcome to my app`}</h1>
         <p>{t`Hello, ${name()}`}</p>

         {/* Rich text with real HTML */}
         <Trans>Read the <a href="/docs">documentation</a> to learn more.</Trans>

         {/* Locale-aware plurals */}
         <Plural value={count()} one="# item in cart" other="# items in cart" />

         <button onClick={() => setLocale('en')}>English</button>
         <button onClick={() => setLocale('zh-CN')}>中文</button>
       </div>
     )
   }
   ```

   No manual keys. No JSON files. `import { t }` is the primary compile-time path, and `useI18n()` stays available for locale switching plus runtime lookup.

5. **Extract messages**

   ```bash
   npx fluenti extract
   ```

   This scans your source files and generates catalog files in `./locales/`.

6. **Add translations**

   Open `locales/zh-CN.po` and fill in `msgstr` fields:

   ```po
   msgid "Welcome to my app"
   msgstr "欢迎使用我的应用"

   msgid "Hello, {name}!"
   msgstr "你好，{name}！"
   ```

7. **Compile messages**

   ```bash
   npx fluenti compile
   ```

8. **Initialize from compiled output**

   ```tsx
   // src/App.tsx
   import { I18nProvider } from '@fluenti/solid'
   import { Home } from './Home'
   import en from './locales/compiled/en'
   import zhCN from './locales/compiled/zh-CN'

   export function App() {
     return (
       <I18nProvider
         locale="en"
         fallbackLocale="en"
         messages={{ en, 'zh-CN': zhCN }}
       >
         <Home />
       </I18nProvider>
     )
   }
   ```

</Steps>

## Key differences from Vue

- Uses `<I18nProvider>` instead of a plugin
- Signals (`name()`) instead of refs (`name.value`)
- JSX instead of templates — use `t` inline in JSX expressions
- Naturally reactive — JSX expressions that call `t` re-run inside Solid's reactive scopes

## Rich text with `<Trans>`

`@fluenti/solid/components` remains available when you want an explicit runtime-only component import path, but the default authoring path is the main `@fluenti/solid` entry.

```tsx
<Trans>Click <a href="/next">here</a> to continue.</Trans>
<Trans>This is <strong>important</strong> information.</Trans>
```

## Plurals with `<Plural>`

```tsx
<Plural value={count()} zero="No items" one="# item" other="# items" />
```

## Next steps

- [SolidJS SPA guide](/frameworks/solid/solid-spa/) — deeper patterns for Solid SPAs
- [SolidStart](/start/quick-start-solidstart/) — SSR with SolidStart
- [Translating Content](/essentials/translating-content/) — all translation APIs

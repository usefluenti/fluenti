---
title: "@fluenti/core"
description: "Complete API reference for @fluenti/core — ICU parser, compiler, interpolation, CLDR plural rules, date/number formatters, and SSR utilities."
---

The framework-agnostic core engine. Zero runtime dependencies.

## `createFluentiRuntime(config)`
...
`createFluentiRuntime()` accepts a `FluentiRuntimeConfigFull` object:
...
`createFluentiRuntime()` and `setLocale()` validate that the locale is a non-empty string. An error is thrown if the locale is empty, `undefined`, or not a string:

```typescript
createFluentiRuntime({ locale: '', messages: {} })
// Error: [fluenti] createFluentiRuntime: locale must be a non-empty string, got ""
...
- `getSSRLocaleScript(locale, options?)` — Generate a `<script>` tag that sets `window.__FLUENTI_LOCALE__` for hydration. Pass `{ key }` to use a custom window variable (useful for micro-frontends)
- `getHydratedLocale(fallback?, options?)` — Read the locale set by the SSR script on the client side. Pass `{ key }` to match a custom variable name
...
The three formatters have intentionally different `styles` parameters:

- **`formatNumber`** accepts function callbacks `(locale) => Intl.NumberFormatOptions` because number formats like currency need locale-aware defaults (e.g. USD for `en`, JPY for `ja`).
- **`formatDate`** accepts a special `'relative'` string style that delegates to `formatRelativeTime`, enabling unified date display with a single function.
- **`formatRelativeTime`** has no `styles` parameter because relative time formatting always uses `Intl.RelativeTimeFormat` with automatic unit selection — there are no meaningful named style variants.
...
Fluenti caches `Intl.*` formatter instances and compiled messages internally for performance. For long-running servers or test isolation, use these APIs to control memory.

### `clearAllCaches()`

Clear **all** internal caches in one call. Covers compiled messages (LRU), `Intl.NumberFormat`, `Intl.DateTimeFormat`, `Intl.PluralRules`, `Intl.RelativeTimeFormat`, and standalone formatter caches.

```typescript
import { clearAllCaches } from '@fluenti/core'

// Periodically reclaim memory in a long-running server
setInterval(() => clearAllCaches(), 4 * 60 * 60 * 1000) // every 4 hours

// Or in tests for deterministic state
afterEach(() => clearAllCaches())
```

### `setMessageCacheSize(maxSize)`

Resize the compiled-message LRU cache. Default: `500`. Clears existing entries when the size changes.

```typescript
import { setMessageCacheSize } from '@fluenti/core'

setMessageCacheSize(1000) // increase for apps with many messages
setMessageCacheSize(0)    // disable caching entirely
```

### `clearInterpolationCache()`

Clear only the compiled-message LRU cache (the cache used by `interpolate()`).

### Individual formatter cache clears

For fine-grained control, each formatter has its own clear function:

| Function | Cache cleared |
|----------|--------------|
| `clearCompileCache()` | `Intl.NumberFormat` + `Intl.DateTimeFormat` instances used during ICU compilation |
| `clearPluralCache()` | `Intl.PluralRules` instances |
| `clearNumberFormatCache()` | `formatNumber()` formatter instances |
| `clearDateFormatCache()` | `formatDate()` formatter instances |
| `clearRelativeTimeFormatCache()` | `formatRelativeTime()` formatter instances |

### `DEFAULT_MESSAGE_CACHE_SIZE`

Exported constant (`500`). The default LRU cache capacity for compiled messages.

### Design notes

- The compiled-message cache uses an **LRU** (Least Recently Used) eviction strategy backed by `Map` insertion order, providing O(1) get/set/evict operations.
- All `Intl.*` caches are **unbounded** `Map`s keyed by `locale:options`. In practice they stay small because most apps use a handful of locales and format styles. For long-running servers, call `clearAllCaches()` periodically to bound memory.
- `clearAllCaches()` is safe to call at any time — all formatters and interpolation will re-create their cached instances transparently on the next call.
...
### `normalizeSelectForms(forms)`

Make select form keys safe for ICU MessageFormat. Non-alphanumeric keys are replaced with `case_0`, `case_1`, etc.

```typescript
import { normalizeSelectForms } from '@fluenti/core'

const { forms, valueMap } = normalizeSelectForms({
  'user@example.com': 'Email user',
  'admin': 'Administrator',
  other: 'Unknown',
})
// forms:    { case_0: 'Email user', admin: 'Administrator', other: 'Unknown' }
// valueMap: { 'user@example.com': 'case_0' }
```

Returns `{ forms, valueMap }` where `forms` has safe keys and `valueMap` maps original keys to their replacements. The `other` key is always preserved as-is.

> This is an advanced/internal utility used by the `<Select>` component. Most users won't need it directly.

### `buildICUPluralMessage(forms, offset?)`

Build an ICU plural message string from a forms object. Optionally includes an `offset:N` clause.

### `buildICUSelectMessage(forms)`

Build an ICU select message string from a forms object.

...
- **`argument`** — Simple `{variable}` placeholder
- **`plural`** — `{var, plural, ...}` with `one`, `other`, etc. branches
- **`select`** — `{var, select, ...}` with named branches
- **`selectordinal`** — `{var, selectordinal, ...}` for ordinal plurals
- **`function`** — `{var, functionName, style}` for number/date/custom formatters
- **`octothorpe`** — The `#` placeholder inside plural/selectordinal branches
...
Programmatic API for running the extract + compile pipeline. Useful for dev-mode file watchers.

### `runExtractCompile(options)`

Run `fluenti extract && fluenti compile` as a child process.

```typescript
import { runExtractCompile } from '@fluenti/core'

await runExtractCompile({
  cwd: '/path/to/project',
...
### `createDebouncedRunner(options, delay?)`

Create a debounced runner that collapses rapid calls. Never runs concurrently — if called while running, schedules a rerun after completion.

```typescript
import { createDebouncedRunner } from '@fluenti/core'

const run = createDebouncedRunner({ cwd: process.cwd() }, 300)
// Call run() on file changes — rapid calls are debounced
```

**`DevRunnerOptions`:** `{ cwd: string, onSuccess?: () => void, onError?: (err: Error) => void }`

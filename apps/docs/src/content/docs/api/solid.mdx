...
Use `useI18n()` when you need the full runtime API (`d`, `n`, `locale`, `setLocale`, `te`, `tm`, etc.). For translation-only code, prefer [`import { t }`](#t-compile-time-import) instead.
...
```tsx
import { DateTime } from '@fluenti/solid/components'
...
```tsx
import { NumberFormat } from '@fluenti/solid/components'

---
title: "@fluenti/next"
description: "API reference for @fluenti/next — withFluenti webpack config, I18nProvider, React Server Components support, and server action helpers."
---
...
:::note
Options like `dateFormats`, `numberFormats`, and `fallbackChain` belong in `fluenti.config.ts`, not in `withFluenti()`. The `WithFluentConfig` interface only accepts Next.js-specific options; all i18n config is read from the config file.
:::

---
title: "@fluenti/react"
description: "API reference for @fluenti/react — I18nProvider, useI18n hook, Trans/Plural/Select components, tagged templates, and server-side helpers."
---
...
| `missing` | `(locale, id) => string?` | **Deprecated.** Use `onMissingKey` instead |
| `onMissingKey` | `(event: MissingKeyEvent) => string \| undefined \| void` | Called when a translation is missing or fallback locale is used |
...
| `te` | `(key, locale?) => boolean` | Check if a translation key exists in the catalog |
| `tm` | `(key, locale?) => CompiledMessage \| undefined` | Get the raw compiled message without interpolation |
...
| `getI18n` | `() => Promise<FluentiInstanceExtended>` | Get the i18n instance |
...
| `te` | `(key, locale?) => Promise<boolean>` | Check if a translation key exists (async) |
| `tm` | `(key, locale?) => Promise<CompiledMessage \| undefined>` | Get raw compiled message (async) |
...
| `getSSRLocaleScript(locale, options?)` | Generate `<script>` tag to inject locale into HTML. Pass `{ key }` for custom variable |
...
2. A new `createFluentiRuntime()` instance is created with the new locale

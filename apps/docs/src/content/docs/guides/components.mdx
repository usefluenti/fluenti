...
Fluenti provides three components for complex translation patterns. In Vue, all three are **globally registered** by the Fluenti plugin — no imports needed. In React and Solid, import them from `@fluenti/react` or `@fluenti/solid`.

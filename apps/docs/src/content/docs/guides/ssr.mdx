...
With `@fluenti/next`, the loader auto-injects everything: `t``\`\`` is rewritten to `__getServerI18n().t(...)`, and `<Trans>`, `<Plural>`, `<DateTime>`, `<NumberFormat>` are auto-imported from the singleton. No manual imports needed.

...
```vue
<template>
  <p>Created: <DateTime :value="post.createdAt" style="long" /></p>
  <p>Updated: <DateTime :value="post.updatedAt" style="relative" /></p>
</template>
```
</TabItem>
...
```vue
<template>
  <p>Price: <NumberFormat :value="item.price" style="currency" /></p>
  <p>Progress: <NumberFormat :value="0.85" style="percent" /></p>
</template>
```
</TabItem>
...
Custom styles are defined via `dateFormats` and `numberFormats` in the provider configuration. See [Configuration](/essentials/configuration/) for details.

...
description: "How to switch locales in Fluenti — setLocale API, reactive locale state, lazy-loaded message catalogs, and preloading strategies for instant switching."
---
...
:::tip[Cookie name is up to you]
The examples use `locale` as the cookie name. Nuxt apps often use `fluenti_locale`. Pick whatever fits your server-side detection logic — just keep it consistent between client and server to avoid hydration mismatches.
:::

:::caution[Use cookies, not localStorage]
Cookies are visible to both server and client on the first request. `localStorage` is client-only, so the server can't know the preferred locale — leading to a flash of wrong content or a hydration mismatch. See [Best Practices](/advanced/best-practices/) for details.
:::
...
- **React:** The provider creates a new `createFluentiRuntime()` instance, triggering re-renders for all children consuming the context.

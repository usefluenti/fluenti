...
description: "Fluenti's compile-time translation APIs — v-t directive, t tagged template, Trans, Plural, Select, and msg. Zero runtime parsing, works across Vue, React, and SolidJS."
---
...
<Aside type="note">
**Two ways to access `t`:**
- **`import { t } from '@fluenti/…'`** — compile-time import, recommended for translation-only code. The build plugin transforms it at compile time.
- **`const { t } = useI18n()`** — runtime hook, use when you also need `d()`, `n()`, `locale`, `setLocale`, `te()`, `tm()`, or other runtime features from the same destructure.

Both produce the same translated output. Choose based on whether you need the full runtime API.
</Aside>
...
function Badge() {
  const { te, t } = useI18n()
  return te('premium.badge') ? <span>{t('premium.badge')}</span> : <span>Free</span>
}
```
</TabItem>
...
For full API details, see the framework-specific reference pages: [@fluenti/vue](/api/vue/#te--check-translation-existence), [@fluenti/react](/api/react/), [@fluenti/solid](/api/solid/#te--check-translation-existence).

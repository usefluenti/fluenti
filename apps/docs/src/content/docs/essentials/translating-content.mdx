...
<Aside type="tip">
The `t` function returned by `useI18n()` supports both function calls (`t('id', values)`) and tagged templates (`` t`Hello ${name}` ``). The `v-t` directive requires no import — it is a compile-time template transform.
</Aside>
...
The `t` function from `useI18n()` doubles as a tagged template literal. The Vite plugin uses AST scope analysis to detect `t` bindings from `useI18n()` and optimizes them at compile time.

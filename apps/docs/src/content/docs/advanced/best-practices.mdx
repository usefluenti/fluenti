...
`fluenti compile` generates a `messages.d.ts` file alongside the compiled catalogs, providing full type safety for your translations. For the full setup guide, see [TypeScript Integration](/essentials/configuration/#typescript-integration).

### What gets generated

```typescript
// src/locales/compiled/messages.d.ts (auto-generated)
export type MessageId =
  | 'abc123'  // "Hello {name}!"
  | 'def456'  // "{count, plural, one {# item} other {# items}}"
  | 'ghi789'  // "Welcome back"

export interface MessageValues {
  abc123: { name: string | number }
  def456: { count: number }
  ghi789: Record<string, never>
}
```

### Variable type inference

The compiler infers variable types from ICU message syntax:

| ICU pattern | Inferred type |
|---|---|
| `{name}` | `string \| number` |
| `{count, plural, ...}` | `number` |
| `{count, selectordinal, ...}` | `number` |
| `{gender, select, male {...} female {...} other {...}}` | `'male' \| 'female' \| 'other'` |

### IDE autocompletion

Add the generated declarations to your `tsconfig.json`:
...
Your editor will autocomplete message IDs and flag incorrect variable types at compile time.
...
Set `strictThreshold: 90` in `fluenti.config.ts` to enforce a minimum translation percentage during `fluenti stats`.

...
|---------|---------------|---------------|
| Fluenti | Built-in (static + dynamic per-route) | `splitting: 'dynamic'` in config |
...
Fluenti's Vite plugin can split translation messages per route automatically. Each locale chunk loads on demand via dynamic imports, so users download only the translations for the current page.

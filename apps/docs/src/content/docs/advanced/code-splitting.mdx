...
description: "Built-in per-route code splitting for i18n — Fluenti is the only library with automatic locale chunking. Choose static, dynamic, or per-route strategies."
---

import { Tabs, TabItem, Aside } from '@astrojs/starlight/components'

`fluenti compile` always generates tree-shakeable locale modules — every message is a `/* @__PURE__ */` annotated export so bundlers can eliminate unused translations. For bundle size optimization and cache tuning, see the [Performance](/advanced/performance/) guide. Fluenti splits this into two separate concerns:

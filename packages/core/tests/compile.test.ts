...
describe('compile', () => {
  ...
  describe('boundary handling', () => {
    ...
    it('coerces non-string custom formatter return to string', () => {
      const ast = parse('{amount, money}')
      const fn = compile(ast, 'en', {
        money: () => 42 as unknown as string,
      }) as (v: Record<string, unknown>) => string
      expect(fn({ amount: 100 })).toBe('42')
    })

    it('coerces null custom formatter return to string', () => {
      const ast = parse('{val, fmt}')
      const fn = compile(ast, 'en', {
        fmt: () => null as unknown as string,
      }) as (v: Record<string, unknown>) => string
      expect(fn({ val: 'x' })).toBe('null')
    })
  })
})

...
function parseAcceptLanguage(header: string): Locale[] {
  // Truncate excessively long headers to prevent CPU abuse
  const safeHeader = header.length > MAX_ACCEPT_LANGUAGE_LENGTH
    ? header.slice(0, MAX_ACCEPT_LANGUAGE_LENGTH)
    : header

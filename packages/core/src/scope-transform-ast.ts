...
import { overwriteNode, throwDirectImportScopeError } from './scope-utils'

import { finalizeImports, injectHelpers } from './scope-helpers'

export interface ScopeTransformAstResult {
  /** Whether the AST was transformed */
  transformed: boolean
  /** Original code (returned as-is when `transformed` is false) */
  code: string
  /** The mutated AST program node (only present when `transformed` is true) */
  ast?: ProgramNode
}

export function scopeTransformAst(
  code: string,
  options: ScopeTransformOptions,
): ScopeTransformAstResult {
  ...
  return { transformed: true, code, ast: program }
}

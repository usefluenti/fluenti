...
describe('collectAllIds', () => {
  ...
  it('includes fuzzy entries (does not skip them)', () => {
    const catalogs: Record<string, CatalogData> = {
      en: { a: { message: 'A' }, fuzzyMsg: { message: 'Fuzzy', fuzzy: true, translation: 'Flou' } },
    }
    expect(collectAllIds(catalogs)).toEqual(['a', 'fuzzyMsg'])
  })
})

describe('compileCatalog with skipFuzzy', () => {
  it('includes fuzzy translations by default', () => {
    const catalog: CatalogData = {
      greeting: { message: 'Hello', translation: 'Bonjour', fuzzy: true },
    }
    const { code, stats } = compileCatalog(catalog, 'fr', ['greeting'])
    expect(code).toContain("'Bonjour'")
    expect(stats.compiled).toBe(1)
  })

  it('skips fuzzy translations when skipFuzzy is set', () => {
    const catalog: CatalogData = {
      greeting: { message: 'Hello', translation: 'Bonjour', fuzzy: true },
    }
    const { code, stats } = compileCatalog(catalog, 'fr', ['greeting'], 'en', { skipFuzzy: true })
    expect(code).toContain('= undefined')
    expect(stats.missing).toContain('greeting')
  })

  it('does not skip non-fuzzy translations when skipFuzzy is set', () => {
    const catalog: CatalogData = {
      greeting: { message: 'Hello', translation: 'Bonjour' },
    }
    const { code, stats } = compileCatalog(catalog, 'fr', ['greeting'], 'en', { skipFuzzy: true })
...
describe('extractMessageVariables', () => {
  it('returns empty array for plain text', () => {
    expect(extractMessageVariables('Hello')).toEqual([])
  })

  it('extracts simple variable as string | number', () => {
    expect(extractMessageVariables('Hello {name}')).toEqual([
      { name: 'name', type: 'string | number' },
    ])
  })

  it('extracts plural variable as number', () => {
    expect(extractMessageVariables('{count, plural, one {# item} other {# items}}')).toEqual([
      { name: 'count', type: 'number' },
    ])
  })

  it('extracts select variable with option key union', () => {
    const vars = extractMessageVariables('{gender, select, male {He} female {She} other {They}}')
    expect(vars).toEqual([
      { name: 'gender', type: "'male' | 'female' | string" },
    ])
  })

  it('extracts multiple variables sorted by name', () => {
    const vars = extractMessageVariables('{greeting} {name}')
    expect(vars).toEqual([
      { name: 'greeting', type: 'string | number' },
      { name: 'name', type: 'string | number' },
    ])
  })

  it('extracts function node variables', () => {
    const vars = extractMessageVariables('{amount, number, currency}')
    expect(vars).toEqual([
      { name: 'amount', type: 'string | number' },
    ])
  })

  it('extracts variables from nested plural options', () => {
    const vars = extractMessageVariables('{count, plural, one {# item by {author}} other {# items}}')
    expect(vars).toEqual([
      { name: 'author', type: 'string | number' },
      { name: 'count', type: 'number' },
    ])
  })
})
...
describe('compileTypeDeclaration', () => {
  it('generates valid TypeScript with MessageId union and MessageValues interface', () => {
    ...
    expect(output).toContain('// Auto-generated by @fluenti/cli')
    expect(output).toContain('export type MessageId =')
    expect(output).toContain("| 'Welcome to Fluenti'")
  ...
  it('handles empty IDs', () => {
    const output = compileTypeDeclaration([], {}, 'en')
    expect(output).toContain('export type MessageId = never')
  })

  it('handles plural messages with number type', () => {
    const catalogs: Record<string, CatalogData> = {
      en: {
    ...
    const allIds = ['{count, plural, one {# item} other {# items}}']
    const output = compileTypeDeclaration(allIds, catalogs, 'en')

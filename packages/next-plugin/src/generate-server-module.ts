...
export function generateServerModule(
  ...
  const sharedFallbackLocales = collectSharedFallbackLocales(locales, defaultLocale, fallbackChain)
...
`
  ...
  for (const locale of locales) {
    ...
    const initialLocales = collectInitialClientLocales(locale, sharedFallbackLocales, locales)
...
function collectSharedFallbackLocales(
  locales: readonly string[],
  defaultLocale: string,
  fallbackChain: Record<string, readonly string[]> | undefined,
): string[] {
  const result = new Set<string>([defaultLocale])
  if (fallbackChain) {
    for (const chain of Object.values(fallbackChain)) {
...
function collectInitialClientLocales(
  locale: string,
  sharedFallbackLocales: readonly string[],
  locales: readonly string[],
): string[] {
  const result = new Set<string>([locale])
  for (const fallbackLocale of sharedFallbackLocales) {
